    ...
    MobileArea.prototype = {
        ...
        bindEvent: function() {
            ...
            function popupArea(e) {
                if ($("body").find(".gearArea").length==0) {
            ...
            function areaCtrlInit() {
                ...
                http({
                    url: '/shop/regions/get',
                    data: {
                        parent_id: 1
                    },
                    success: function(data) {
                        _self.province_list = data.data;
                        _self.setGearTooth(data.data);
                    }
                })
            }
            //触摸开始
        ...
        setGearTooth: function(data) {
            ...
            var gearVal = 0;
            var gearRef = gearChild[_self.index].getAttribute('val');
            for (var i = 0; i < item.length; i++) {
                if(item[i][_self.keys['id']] == gearRef){
                    gearVal = i;
                    break;
                }
            }
            ...
            if (l > 0) {
                var childData;
                var region_id = '';
                if (!prepArr(_self.value, [0,0,0])&&_self.is_first) {
                    region_id = _self.value[_self.index]
                }else{
                ...
                http({
                    ...
                    success: function(data) {
                        ...
                        var itemStr = "";
                        for (var i = 0; i < l; i++) {
                            itemStr += "<div class='tooth'  ref='" + item[i][_self.keys['id']] + "'>" + item[i][_self.keys['name']] + "</div>";
                        }
        ...
        finish: function(e) {
            ...
            vm.area_id = provinceCode + ((cityCode) ? (',' + cityCode) : ('')) + ((countyCode) ? (',' + countyCode) : (''));
            _self.close(e);
        },
        close: function(e) {
            e.preventDefault();
